				...
				success:function(rs){
					...
					for ( var i in apply) {
						...
						if(apply[i].audit_status == 1){
							...
							apply[i].audit_status = '未审核';
						}
						//判断放款按钮状态
						var loanBtn = "";
						if(apply[i].shenHe_status == 1){
							loanBtn = '<button type="button" disabled class="btn btn-success">已审核</button>';
						}else{
							loanBtn = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="fangLoan('+apply[i].loan_order+')">审核</button>';
						}
	 	...
	 	$.ajax({
            ...
            success: function (rs) {
				...
				for ( var i in apply) {
					...
					var loanBtn = "";
					if(apply[i].shenHe_status == 1){
						loanBtn = '<button type="button" disabled="disabled" class="btn btn-success">已审核</button>';
					}else{
						loanBtn = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="fangLoan('+apply[i].loan_order+')">审核</button>';
					}
					//审核结束时间（'yyyy-MM-dd HH:mm:ss'）
